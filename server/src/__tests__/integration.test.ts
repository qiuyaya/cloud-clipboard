...
describe("Integration Tests", () => {
  ...
  describe("Room API", () => {
    it("should return 404 for non-existent room", async () => {
      const response = await request(app).get("/api/rooms/nonexistent").expect(404);
  ...
  describe("Room API Coverage Tests", () => {
    ...
    it("should handle invalid file downloads", async () => {
      await request(app).get("/api/files/download/nonexistent.txt").expect(404);

      await request(app).get("/api/files/download/../../../etc/passwd").expect(404);
    });

    it("should handle invalid file deletions without auth", async () => {
      await request(app).delete("/api/files/nonexistent.txt").expect(401);
    });
  ...
  describe("Error Handling", () => {
    it("should handle invalid routes", async () => {
      ...
      await request(app).post("/api/invalid-endpoint").expect(404);
    });

    it("should handle malformed requests gracefully", async () => {
      // Test malformed file upload without authentication
      const response = await request(app).post("/api/files/upload").send("not a file");
    ...
    it("should handle large request headers", async () => {
      ...
      const response = await request(app).get("/api/health").set("X-Large-Header", largeHeader);
    ...
    it("should handle different content types", async () => {
      ...
      await request(app).get("/api/health").set("Accept", "application/json").expect(200);

      await request(app).get("/api/health").set("Accept", "text/plain").expect(200);
    });
    ...
    it("should test API with different user agents", async () => {
      ...
      for (const userAgent of userAgents) {
        const response = await request(app).get("/api/health").set("User-Agent", userAgent);
