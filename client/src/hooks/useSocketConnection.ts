...
export const useSocketConnection = ({
  ...
  useEffect(() => {
    ...
    const handleConnect = async () => {
      ...
      if (savedUser && savedRoomKey) {
        ...
        try {
          ...
          debug.error('Failed to validate user, proceeding with normal join', { error });
          onSetIsConnecting(true);
          onSetCurrentUser(null);
          onSetUsers([]);
          onSetMessages([]);
    ...
    const handleMessage = (message: TextMessage | FileMessage) => {
      ...
      onSetMessages(prev => [...prev, messageWithDate]);
    };
    ...
    const handleRoomPasswordSet = (data: { roomKey: string; hasPassword: boolean }) => {
      onSetHasRoomPassword(data.hasPassword);
      toast({
