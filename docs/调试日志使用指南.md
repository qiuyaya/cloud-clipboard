...
Rust 后端日志通过 `RUST_LOG` 环境变量进行配置：

```bash
# 日志级别配置
export RUST_LOG=cloud_clipboard_server=debug,tower_http=debug   # 显示所有调试信息
export RUST_LOG=cloud_clipboard_server=info,tower_http=info     # 信息级别（默认）
export RUST_LOG=cloud_clipboard_server=warn,tower_http=warn     # 只显示警告和错误
export RUST_LOG=cloud_clipboard_server=error                     # 只显示错误
```
...
docker run -p 3001:3001 -e RUST_LOG=cloud_clipboard_server=debug,tower_http=debug cloud-clipboard-rust
```
